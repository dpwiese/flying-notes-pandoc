...
<!--
[AIM 5-4-9(a)(5)](https://www.faa.gov/air_traffic/publications/atpubs/aim_html/chap5_section_4.html)
...
![IFR chart index. [IFR Enroute Aeronautical Charts and Planning](https://www.faa.gov/air_traffic/flight_info/aeronav/digital_products/ifr/)](./img/ifr-chart-index.jpg){width=5in}

![IFR chart legend altitude symbols](./img/ifr-chart-legend-altitude-symbols.png){width=2.5in}
...
<!--
TODO@dpwiese
* Consider also `TOMAC` intersection and `KEMAR`. `TOMAC` has the little arrow indicating `HGR` forms the fix. But `KEMAR` does not have the arrow from `HGR` meaning `HGR` doesn't define `KEMAR`?
* Operationally, do the DME arrows matter at all since planes pretty much just use GPS in lieu of DME?
-->
...
<!--
  * Position reports
    * Position reports are required over each compulsory reporting point (shown on the chart as a solid triangle) along the route being flown
...
<!--
  * I could not find any information specifically about this so the following are some points about radar coverage
  * First note that there are differences between weather radar and surveillance radar
  * Surveillance radar
    * Primary radar displays information about reflected returns, and cannot distinguish from what the returns are from
    * Secondary radar uses the return from the transponder to associate radar returns with a particular aircraft
      * And in the case of mode-C its altitude
  * Weather radar
    * [AC 00-45H Aviation Weather Services](https://www.faa.gov/regulations_policies/advisory_circulars/index.cfm/go/document.information/documentID/1030235) has a figure showing where weather radar stations are
  * The below quote was from an old Instrument Flying Handbook, I think

  > In addition, most en route flights are tracked by radar along their entire route in the 48 contiguous states, with essentially total radar coverage of all instrument flight routes except in the mountainous West. Lack of radar coverage may be due to terrain, cost, or physical limitations.

  * ==There are differences between weather radar and surveillance radar==
  * ==Figure out what capabilities exist where you are flying==
  * ==If uncertain about radar coverage, be prepared to make required reports if needed==


AIM FIG 4-5-7 - Terminal Mode S Radar Sites
AIM 4-5-3 Surveillance Radar
ASR vs ARSR

FAA-AC-00-45H has Figure 3-6 showing where weather radars are
-->
...
![Aerial image of ground checkpoint at Richmond.](./img/kric-vor-ground-checkpoint.jpg){width=6in}
...
<!--
https://aviation.stackexchange.com/questions/46233/is-the-standby-altimeter-required-for-day-vfr-flight-in-a-c172-nav-iii

Use example of standby altimeter in the above

https://www.ecfr.gov/current/title-14/chapter-I/subchapter-F/part-91/subpart-B/subject-group-ECFRe4c59b5f5506932/section-91.121

* Basically says set the altimeter as best you can to within 100 nm of where you are in flight
* Note radio is not required for day VFR, which in that case can set it to field elevation upon takeoff
* Still can't find more on requirements for altimeter under day VFR

https://www.ecfr.gov/current/title-14/chapter-I/subchapter-C/part-23/subpart-F/section-23.2500

* I think the answer is, for day VFR in a plane without KOEL/CEL, whatever altimeter the plane has that it was certified with must be operable.
* I don't think if the altimeter was broken you can tag it as INOP and then use GPS altitude from a portable GPS, for example.
* In terms of what minimally the FAA would certify as an altimeter I can't really find - but it's hard to imagine a scenario in which a plane would *not* have an altimeter and static system that is also compliant with the requirements for IFR, except without requiring the 24 month inspection.
-->
